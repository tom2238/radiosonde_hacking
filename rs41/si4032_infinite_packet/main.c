...
int main(void) {
    ...
    // Infinite packet (over 255 bytes), 4800 baud, 2400 Hz deviation, packet size dont care, 80 nibbles dont care
    Si4032_PacketMode(PACKET_TYPE_INFINITE,4800,2400,0xFF,80);
	...
	while (1) {
        ...
        for(i=0;i<15;i++) {
            // Write 20 bytes into FIFO
            Si4032_WritePacketData(dataframe.value,i*20,20);
            // Wait for empty FIFO;
            while(!Si4032_IsFIFOEmpty());
        }
        // Write 20 bytes into FIFO
        Si4032_WritePacketData(dataframe.value,300,20);

        // Total 360 bytes (preamble(40) + head(8) + data(310) + crc(2))
        // Wait for send
